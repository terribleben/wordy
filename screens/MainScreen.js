...
export default class MainScreen extends React.Component {
  ...
  render () {
    ...
    if (loading) {
      ...
      return this._renderWords(this.state.words, this.state.cloud);
    }
  }
  ...
  _resetAsync = async () => {
    ...
    await this._computeCloudAsync(this.state.words);
    if (this._mounted) {
  ...
  _computeBoundingBox = (word, weight, existingBoxes) => {
    ...
    if (!existingBoxes || existingBoxes.length == 0) {
      ...
      return {
        x: (Dimensions.get('window').width * 0.5) - (width * 0.5),
        y: (Dimensions.get('window').height * 0.5) - (height * 0.5),
        width,
        height,
      };
    } else {
